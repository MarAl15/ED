...
Pila_max::Pila_max(const VD <elemento> pila){
...
void Pila_max::poner(const int dato){
	...
	if(pila_max.vacia() || pila_max[num_elementos-1].maximo < dato)
		elem_aux.maximo = dato;
	else
		elem_aux.maximo = pila_max[num_elementos-1].maximo;
	

	// Se añade el nuevo elemento en la última posición.
	pila_max.poner(elem_aux, num_elementos);
...
elemento & Pila_max::tope(){
	...
	return pila_max[pila_max.size()-1];
}
